...
class SuperModel{
    ...
    public static function add_response($selected_province_id,$name,$address,$first_name,$comments,$contactno,$district_id) {
        ...
        try {
            ...
            $query1 = "INSERT INTO `response` (`citizen_name`, `citizen_phone`, `Citizen_address`, `Response`, `p_idd`, `d_idd`, `t_idd`) VALUES ('$name', '$contactno', '$address', '$comments', '$selected_province_id', '$district_id', '$district_id');";
     ...
     public static function add_user($name,$username,$password) {
        //the below function creates a teacher in the database
        ...
        try {
            ...
            //Insets data into user master tabble
            $query1 = "INSERT INTO `login` (`Username`, `Name`, `Password`, `isActive`) VALUES ('$name', '$username', '$password', '1');";
     ...
     public static function get_responses_by_procince_id($province_id) {
       
        $Connection = new Connection();
        $conn = $Connection->connect();

        $query = "CALL GetResponsesByPovinceID('$province_id');";
   ...
   function get_districts_by_provinceid($provinceid) {
     ...
     function get_districts_by_provinceid_response_one($provinceid) {
        //This function is used to load the districts whih a given province ID
         $Connection = new Connection();
        $conn = $Connection->connect();
        ...
        if ($stm->rowCount() > 0) {
            ...
            echo "  <div class=''>  <div >    <select onChange=get_responses_one() class='form-control' id='district_id_one' name='district_id' required='required' ><option value='' selected disabled=''>Select District</option>";
            while ( $row = $stm->fetch(PDO::FETCH_ASSOC)) {
     ...
     function get_districts_by_provinceid_respone_tow($provinceid) {
        //This function is used to load the districts whih a given province ID
         $Connection = new Connection();
        $conn = $Connection->connect();
        ...
        if ($stm->rowCount() > 0) {
            ...
            echo "  <div class=''>  <div >    <select onChange=get_responsestwo() class='form-control' id='district_id_two' name='district_id' required='required' ><option value='' selected disabled=''>Select District</option>";
            while ( $row = $stm->fetch(PDO::FETCH_ASSOC)) {
